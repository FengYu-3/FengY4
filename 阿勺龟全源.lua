...
makeplrbutton("他的好友").MouseButton1Click:Connect(function()
	...
	for i, v in pairs(plrs:GetPlayers()) do
        ...
        local charframe = makeFrame(scrollingFrame, "秒开区域", Color3.fromRGB(53, 53, 100))
	...
	infjumpb.Text = "连跳" .. ((infjump and "(开)") or "")
        ...
        autoore.MouseButton1Click:Connect(function()
            if autooret == false then
                autooret = true
                autoore.Text = "自动捡各种稀有珠宝(开)"
            else
                autooret = false
                autoore.Text = "自动捡各种稀有珠宝"
            end
        ...
        local automgoldgun = makeutilbutton("自动捡玫瑰")
            ...
            for i,l in pairs(game:GetService("Workspace").Game.Entities.ItemPickup:GetChildren()) do
                for i,v in pairs(l:GetChildren()) do
                    if v.ClassName == "MeshPart" or "Part" then
                        for i,e in pairs(v:GetChildren()) do
                            if e.ClassName == "ProximityPrompt" then
                                if e.ObjectText == "Black Rose" or e.ObjectText == "Golden Rose" then
                                    local epoh1 = v.CFrame
                                    epoh3.CFrame = epoh1 * CFrame.new(0, 2, 0)
                                    wait(0.1)
                                    e:InputHoldBegin()
        ...
        local autoPresent = makeutilbutton("自动宝藏地图")
        local autoPresentt = false
        autoPresent.MouseButton1Click:Connect(function()
            if autoPresentt == false then
                autoPresentt = true
        ...
        makeutilbutton("金保险").MouseButton1Click:Connect(function()
            local BankDoor = game:GetService("Workspace").BankRobbery.VaultDoor
            local epoh2 = game:GetService("Players")
            local epoh3 = epoh2.LocalPlayer.Character.HumanoidRootPart
            if BankDoor.Door.Attachment.ProximityPrompt.Enabled == true then
        ...
        makeutilbutton("黑保险").MouseButton1Click:Connect(function()
            local BankDoor = game:GetService("Workspace").BankRobbery.VaultDoor
            local epoh2 = game:GetService("Players")
            local epoh3 = epoh2.LocalPlayer.Character.HumanoidRootPart
            if BankDoor.Door.Attachment.ProximityPrompt.Enabled == true then
        ...
        makeutilbutton("海盗大宝箱").MouseButton1Click:Connect(function()
        ...
        makeutilbutton("海盗小宝箱").MouseButton1Click:Connect(function()
        ...
        makeutilbutton("小保险").MouseButton1Click:Connect(function()
        ...
        makeutilbutton("挖宝藏").MouseButton1Click:Connect(function()
        ...
        local cbringframe = makeFrame(scrollingFrame, "死亡笔记", Color3.fromRGB(53, 53, 100))
        			...
        			for _, player in pairs(players:GetPlayers()) do
        				...
        				if character then
        					...
        					if gta then
        						...
        						gta.HoldDuration = 0
        						gta.ActionText = "lyy觉得你可以把Ta抓起来，或者踩死"
        						local leaderstats = Instance.new("IntValue")
        						leaderstats.Name = "leaderstats"
        						leaderstats.Value = player.stats.Money.Value
